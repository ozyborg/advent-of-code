def print_registration_identifier(data)
  char_map = [' ', '*']

  xs = data.keys.map { |k| k[0] }
  ys = data.keys.map { |k| k[1] }

  (ys.max).downto(ys.min).map do |y|
    (xs.min..xs.max).map do |x|
      char_map[data.fetch([x, y], []).last || 0]
    end.join
  end.join("\n")
end
...
def process(data)
  ...
  print_registration_identifier(position_colors)
end
