...
def process(data)
  programs = 50.times.map do |i|
    [i, data.dup + Array.new(1000){0}]
  end.to_h
  queues = 50.times.map do |i|
    [i, []]
  end.to_h
  stack = []
  ...
  loop do
    ...
    50.times do |i|
      ...
      if queues.values.flatten.empty?
        if last_y == nat_y
          res = last_y
          break
        else
        ...
        queues[0].unshift(nat_y)
        queues[0].unshift(nat_x)
      end
    end
